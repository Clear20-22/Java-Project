...
//        menu_gridPane.setVgap(15);
    ...
    void showProfileInformation(){
        ...
        try {
            String sql = "SELECT FullName, RegistrationID, Category, PhoneNumber, Department, Password, PasswordClue FROM userdetails WHERE RegistrationID = ?";
            ...
            if (resultSet.next()) {
                ...
                clue = resultSet.getString("PasswordClue");
                num = phone;
    ...
    void ProfileUpdate(ActionEvent event) {
        ...
        if(opt.isPresent() && opt.get() == ButtonType.OK){
            ...
            update_passclue.setText(clue);
            update_password.setText(pass);
            if(num != null){
                updatePhone.setText(num);
            }
        }
    ...
    @FXML
    private TextField update_passclue;
    ...
    void confirm_update(){
        ...
        String passclue = update_passclue.getText();

        if (name.isEmpty() || department.isEmpty() || phone.isEmpty() || passclue.isEmpty() || password.isEmpty()) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
        ...
        try {
            String updateQuery = "UPDATE userdetails SET FullName = ?, Department = ?, PhoneNumber = ?, Password = ?, PasswordClue = ? WHERE RegistrationID = ?";
            ...
            pstmt.setString(5, passclue);
            pstmt.setString(6, const_ID.getText());
   ...
   private int Cid;
    ...
    private void showCustomerData() {
        ...
        centerAlignColumn(pars_col_customerid);
        centerAlignColumn(pars_col_customername);
        centerAlignColumn(pars_col_price);
        centerAlignColumn(pars_col_quantity);
        centerAlignColumn(pars_col_date);
    }

    private <T> void centerAlignColumn(TableColumn<CustomerSummary, T> column) {
