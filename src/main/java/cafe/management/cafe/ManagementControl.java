               ...
               try {
                    ...
                    if (resultSet.next()) {
                        ...
                        if(username.equals("admin")){
                            CafeManagement.switchScene("Admin.fxml", 1100);
...
//                date = convertDate(date,"dd-MM-yyyy");
                String query = "UPDATE userdetails SET password = ? WHERE RegistrationID = ? AND PasswordClue = ?";
                ...
                // Show the alert and wait for OK click
    ...
    void ForgetProceed(ActionEvent event) {
        ...
        String date = ForgateDate.getText();
        if(email.isEmpty()){
            blankemail.setText("ID is required.");
            return;
        }

        if(date.isEmpty()){
            ForgateDateError.setText("Password Clue is required.");
            return;
        }

        try{
//            date = convertDate(date,"dd-MM-yyyy");
            String query = "SELECT * FROM userdetails WHERE RegistrationID = ? AND PasswordClue = ?";
            preparedStatement = connection.prepareStatement(query);
            ...
            if(resultSet.next()){
                ...
                blankemail.setText("Use Valid ID.");
                ForgateDateError.setText("Use Valid Clue.");
                GiveForgetEmail.setText("");
                ForgateDate.setText("");
            }
        }catch (SQLException e){
